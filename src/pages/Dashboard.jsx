      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 pb-6">
        ...
        <div className="glass-panel p-6 lg:col-span-2">
          <div className="flex justify-between items-start mb-6">
            <div>
              <h2 className="text-lg font-bold text-gray-800">Enrolment Habits</h2>
              <p className="text-xs text-gray-500 mt-1">Track your enrolment habits</p>
            </div>
            <select className="text-xs bg-gray-50 border border-gray-200 text-gray-600 rounded-md px-2 py-1 outline-none font-medium">
          ...
          <div className="h-[280px] w-full mt-4">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={barData} margin={{ top: 0, right: 0, left: -20, bottom: 0 }} barGap={6}>
                ...
                <Bar dataKey="value1" fill="#064e3b" radius={[4, 4, 0, 0]} barSize={16} />
                <Bar dataKey="value2" fill="#22c55e" radius={[4, 4, 0, 0]} barSize={16} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="space-y-6">
          <div className="glass-panel p-6 bg-gradient-to-br from-green-50/50 to-emerald-100/30 border-green-100/50">
            <div className="flex justify-between items-start mb-4">
              <div>
                <h2 className="text-lg font-bold text-gray-800">Risk Statistic</h2>
                <p className="text-xs text-gray-500 mt-1">Track your risk levels</p>
              </div>
              <select className="text-xs bg-transparent text-gray-600 outline-none font-medium">
                <option>Today</option>
              </select>
            </div>
          ...
          <div className="glass-panel p-5">
            <div className="flex justify-between items-start mb-4">
              <div>
                <h2 className="text-sm font-bold text-gray-800">Regional Growth</h2>
            ...
            <div className="flex items-center justify-between mt-2">
              ...
              <div className="relative w-24 h-24">
                <div className="absolute top-0 left-0 w-14 h-14 bg-brand-green rounded-full flex items-center justify-center text-white text-xs font-bold shadow-sm">87%</div>
                <div className="absolute bottom-0 right-0 w-12 h-12 bg-brand-lightgreen rounded-full flex items-center justify-center text-white text-[10px] font-bold shadow-sm">57%</div>
                <div className="absolute bottom-1 left-2 w-9 h-9 bg-brand-green/80 rounded-full flex items-center justify-center text-white text-[9px] font-bold shadow-sm">37%</div>
                <div className="absolute top-1 right-1 w-7 h-7 bg-brand-lightgreen/80 rounded-full flex items-center justify-center text-white text-[8px] font-bold shadow-sm">17%</div>
              </div>
              <div className="space-y-3 text-[10px] font-medium text-gray-500 pr-2">
                 <div className="flex items-center gap-2"><span className="w-3 h-2 rounded-sm bg-brand-green"></span>Lagos</div>
                 <div className="flex items-center gap-2"><span className="w-3 h-2 rounded-sm bg-brand-lightgreen"></span>Abuja</div>
...
const StatCard = ({ title, value, icon: Icon, change, isPositive }) => (
  <div className="glass-panel p-5 shadow-sm">
    <div className="flex justify-between items-start mb-4">
      ...
      <span className={`text-[10px] font-bold px-2 py-1 rounded-full text-white ${isPositive ? 'bg-brand-lightgreen' : 'bg-red-500'}`}>
        {change}
      </span>
    </div>
